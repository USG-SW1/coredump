...
DAEMON_WHITELIST = _config.get("daemon-whitelist", [])
...
def extract_daemon_from_target(target):
    """從 target basename 取得 daemon 名稱（第一個 '#' 到第二個 '#' 之間）。

    Example: 260413-184509_1.38p0_ABWV#isc-worker0000#14300.libdhcp.core.zip
    → daemon: isc-worker0000
    """
    if not target:
        return ""
...
def extract_coredump_key(target):
    ...
    Use the part after the second '#' (actual coredump filename), then strip
    everything up to and including the first '-' to get the unique key.

    Example: 260413-184509_1.38p0_ABWV#isc-worker0000#1bc90-nginx.core.zip
    → coredump-key: nginx.core.zip
    """
    if not target:
        return ""
    filename = os.path.basename(target)
    parts = filename.split('#', 2)
    suffix = parts[2] if len(parts) == 3 else (parts[0] if parts else filename)
    idx = suffix.find('-')
    if idx >= 0:
        return suffix[idx + 1:]
    return suffix
...
def extract_coredump_files(target):
    """從 target 路徑取得完整檔名（basename，含日期時間，維持到 .core.zip）"""
...
def is_daemon_whitelisted(daemon):
    """檢查 daemon 是否在白名單中（支援正則表達式或完整匹配）"""
    if not daemon or not DAEMON_WHITELIST:
        return False
    daemon_lower = daemon.lower()
    for pattern in DAEMON_WHITELIST:
        pattern_lower = pattern.lower()
        # Try exact match first
        if daemon_lower == pattern_lower:
            return True
        # Try regex match
        try:
            if re.match(pattern_lower, daemon_lower):
                return True
        except re.error:
            pass
    return False
...
def step2_first_confirm(status, logger):
    ...
    if need_confirm > 0:
        ...
        for _, row in pending:
            ...
            is_its = is_its_firmware(firmware)
            sn = row.get("sn", "").strip()
            daemon = extract_daemon_from_target(target) or row.get("daemon", "").strip()
    ...
    for current, (i, row) in enumerate(pending, 1):
        ...
        # ── Check daemon whitelist ──
        if is_daemon_whitelisted(daemon):
            logger.log(f"[白名單] Daemon '{daemon}' 在白名單中，自動跳過")
        ...
        # At this point, is_its is always False
        default = "y"
        choice = ask_user(
        ...
        if choice == "q":
            ...
            # Extract datetime prefix: first 13 chars of target filename (e.g. 260226-131747)
            target_basename = os.path.basename(target)
            datetime_prefix = target_basename[:13] if len(target_basename) >= 13 else ""
...
def _download_from_optools(page, sn, mac, sh_script_path, logger):
    """Use OpTools to upload shell script and poll for download link."""
    ...
    def on_response(resp):
        if "dev-troubleshoot" in resp.url:
            try:
                body = resp.text()
                logger.log(f"  [NET] Response: {resp.status} {resp.url} | body={body[:300]}")
            except Exception:
    ...
    # ── Upload shell script ──
    if sh_script_path and os.path.exists(sh_script_path):
        logger.log(f"  Uploading shell script: {sh_script_path}")
        try:
            # Find file input on the page (selector may need adjustment)
            file_inputs = page.locator("input[type='file']")
            count = file_inputs.count()
            logger.log(f"  [DEBUG] Found {count} file input(s) on page")
            for i in range(count):
                inp = file_inputs.nth(i)
                inp_id = inp.get_attribute("id") or ""
                inp_name = inp.get_attribute("name") or ""
                logger.log(f"  [DEBUG] file input[{i}]: id='{inp_id}' name='{inp_name}'")

            # Use the script-upload file input
            page.locator("#script-upload").first.set_input_files(sh_script_path)
            logger.log(f"  [OK] Shell script set to file input")
            page.wait_for_timeout(1000)

            # Log current file input value for confirmation
            input_val = page.evaluate("""(() => {
                const inp = document.querySelector("input[type='file']");
                return inp ? (inp.files.length > 0 ? inp.files[0].name : 'no file') : 'not found';
            })()""")
            logger.log(f"  [DEBUG] File input value: {input_val}")
        except Exception as e:
            logger.log(f"  [WARN] Failed to upload shell script: {e}")
    else:
        logger.log(f"  [WARN] Shell script not found: {sh_script_path}, skipping upload")

    # ── Trigger upload button ──
    logger.log("  Calling trigger_device_upload_log()...")
    # Get previous key_path to detect if it changes (indicates successful upload)
    ...
    # Verify key_path actually changed from previous upload
...
def _prepare_shell_script(datetime_prefix, logger):
    """Copy temp.zysh, replace <date-time> with datetime_prefix (no trailing whitespace/newline),
    save as <datetime_prefix>.zysh.

    Returns the path to the generated script, or None on failure.
    """
    template_path = os.path.join(SCRIPT_DIR, "temp.zysh")
    if not os.path.exists(template_path):
        logger.log(f"  [FAIL] temp.zysh not found at {template_path}")
        return None

    script_name = f"{datetime_prefix}.zysh"
    ...
    try:
        ...
        # Replace <date-time> and ensure no trailing whitespace/newline after it
        replaced = content.replace("<date-time>", datetime_prefix)
        # Strip trailing whitespace and newlines from the entire file
        replaced = replaced.rstrip()
        logger.log(f"  [DEBUG] Shell script content after replace:\n{replaced}")

        with open(script_path, "w", encoding="utf-8") as f:
            f.write(replaced)

        logger.log(f"  [OK] Shell script prepared: {script_path}")
        return script_path
    except Exception as e:
        logger.log(f"  [FAIL] Failed to prepare shell script: {e}")
        return None
...
def _verify_coredump(filepath, coredump_key, logger):
    ...
    NOTE: zip verification is currently SKIPPED (always returns True).
    Re-enable the checks below when needed.

    Returns (ok, zip_listing) tuple.
    """
    logger.log(f"  [SKIP] zip verification skipped, assuming OK")
    return True, ""

    # ── DISABLED: zip verification checks ──
    # import subprocess
    # import zipfile
    #
    # zip_filename = os.path.basename(filepath)
    # expected_inner = zip_filename[:-4] if zip_filename.lower().endswith(".zip") else zip_filename
    #
    # logger.log(f"  [DEBUG] zip file path:      {filepath}")
    # logger.log(f"  [DEBUG] zip filename:        {zip_filename}")
    # logger.log(f"  [DEBUG] expected inner name: {expected_inner}")
    # logger.log(f"  [DEBUG] coredump key:        {coredump_key}")
    #
    # # ── Run unzip -l for human-readable debug log ──
    # try:
    ...
    #     logger.log(f"  [DEBUG] unzip -l output:\n{listing}")
    # except Exception as e:
    #     listing = str(e)
    #     logger.log(f"  [WARN] unzip -l failed: {e}")
    #
    # # ── Parse zip entries with zipfile module ──
    # try:
    #     with zipfile.ZipFile(filepath, 'r') as zf:
    #         entries = [info.filename for info in zf.infolist() if not info.is_dir()]
    #     logger.log(f"  [DEBUG] zip entries ({len(entries)}): {entries}")
    # except Exception as e:
    #     logger.log(f"  [FAIL] Cannot open zip: {e}")
    #     return False, listing
    #
    # # Check 1: coredump-key present
    # if coredump_key and not any(coredump_key in e for e in entries):
    #     logger.log(f"  [FAIL] Coredump key '{coredump_key}' not found in zip entries")
    #     return False, listing
    # logger.log(f"  [OK] Coredump key found in zip")
    #
    # # Check 2: exactly one file
    # if len(entries) != 1:
    #     logger.log(f"  [FAIL] Expected exactly 1 file in zip, got {len(entries)}: {entries}")
    #     return False, listing
    # logger.log(f"  [OK] Zip contains exactly 1 file")
    #
    # # Check 3: inner filename can be found within zip filename (substring match)
    # inner_name = os.path.basename(entries[0])
    # logger.log(f"  [DEBUG] inner filename:      {inner_name}")
    # if inner_name not in zip_filename:
    #     logger.log(f"  [FAIL] Inner filename '{inner_name}' not found in zip name '{zip_filename}'")
    #     return False, listing
    # logger.log(f"  [OK] Inner filename '{inner_name}' found in zip name")
    #
    # return True, listing
    ...
    with sync_playwright() as p:
        ...
        try:
            for idx, sn in enumerate(unique_sns, 1):
                ...
                # Prepare shell script for this SN
                primary_for_sh = next(
                ...
                sh_script_path = None
                if primary_for_sh:
                    dt_prefix = primary_for_sh.get("datetime_prefix", "")
                    logger.log(f"  datetime_prefix: {dt_prefix}")
                    if dt_prefix:
                        sh_script_path = _prepare_shell_script(dt_prefix, logger)
                    else:
                        logger.log(f"  [WARN] datetime_prefix 為空，無法準備 shell script")

                # Get download link
                try:
                    link = _download_from_optools(page, sn, mac, sh_script_path, logger)
                except Exception as e:
                    error_msg = str(e)
                    logger.log(f"  [FAIL] {error_msg}")
                    for rec in status["records"]:
...
def step4_post_jira(status, logger):
    ...
    for idx, rec in enumerate(need_confirm, 1):
        ...
        try:
            ...
            # 更新 Coredump URL (含 jira_id，與 Model/versions 合併同一個 PUT)
            if 'Coredump URL' in field_map:
                year = datetime.now().strftime("%Y")
                ftp_url = f"ftp://{_config['ftp-host']}/jira/{year}_ML_Coredump_files/{jira_id}/{coredump_files}"
                put_fields[field_map['Coredump URL']] = ftp_url
                logger.log(f"  Coredump URL: {ftp_url}")
            else:
                logger.log(f"  [WARN] 找不到 'Coredump URL' 欄位，無法更新")

            # 只有在 put_fields 不是空的時候才更新
            if put_fields:
                try:
                    update_fields(_config, jira_id, put_fields, logger=logger)
                    logger.log(f"  已更新 Model、Affects Version、Coredump URL")
                except Exception as e:
                    logger.log(f"  [WARN] 更新欄位失敗: {e}")
...
def update_jira_count(logger):
    ...
    for row in data_rows:
        ...
        cnt = 1 + related_counter.get(jira_id, 0)  # self + duplicates
            ...
            if xlsx_jira_id_idx is None:
                ...
                for row_idx in range(2, ws.max_row + 1):
                    ...
                    if re.match(r'^ZNGA-\d+$', jira_id_str):
                        cnt = 1 + related_counter.get(jira_id_str, 0)  # self + duplicates
                        ws.cell(row=row_idx,
...
def step7_report(logger):
    ...
    problems = []
    jira_status_map = {}  # jira_id -> status_text
    ...
    for idx, jid in enumerate(unique_ids, 1):
        ...
        if exists:
            ...
            jira_status_map[jid] = status_text
        else:
            jira_fail += 1
            j_s = "NOT FOUND"
            jira_status_map[jid] = ""
    ...
    # ── Print fixed (non-Open) Jira IDs ──
    fixed_ids = [(jid, jira_status_map[jid]) for jid in unique_ids
                 if jira_status_map.get(jid) and jira_status_map[jid] != "Open"]
    if fixed_ids:
        logger.log(f"\n--- Fixed Jira IDs ({len(fixed_ids)}) ---")
        for jid, st in fixed_ids:
            col, sn, daemon = jira_ids[jid][0]
            logger.log(f"  {jid}  Status: {st}  (SN: {sn}, daemon: {daemon})")
    else:
        logger.log(f"\n--- Fixed Jira IDs: 無 ---")

    # ── Update Status column in ELK-summary.xlsx ──
    xlsx_path = _elk_query.SUMMARY_XLSX
    if jira_status_map and os.path.exists(xlsx_path):
        try:
            wb = openpyxl.load_workbook(xlsx_path)
            ws = wb['in'] if 'in' in wb.sheetnames else wb.active
            ...
            jira_id_col = header_map.get('jira-id')
            related_col = header_map.get('related-jira-id')
            status_col = header_map.get('Status')

            if not status_col:
                logger.log("  [WARN] XLSX 中找不到 'Status' 欄位，跳過狀態更新")
            else:
                xlsx_updated = 0
                for row_idx in range(2, ws.max_row + 1):
                    # Check jira-id column
                    jid = None
                    if jira_id_col:
                        val = ws.cell(row=row_idx, column=jira_id_col).value
                        if val and str(val).strip() in jira_status_map:
                            jid = str(val).strip()
                    # Fallback: check related-jira-id
                    if not jid and related_col:
                        val = ws.cell(row=row_idx, column=related_col).value
                        if val and str(val).strip() in jira_status_map:
                            jid = str(val).strip()
                    if jid:
                        st = jira_status_map[jid]
                        if st:
                            ws.cell(row=row_idx, column=status_col, value=st)
                            xlsx_updated += 1
                wb.save(xlsx_path)
                logger.log(f"  已更新 XLSX Status 欄位 {xlsx_updated} 筆")
        except Exception as e:
            logger.log(f"  [WARN] 更新 XLSX Status 失敗: {e}")
...
def refresh_xlsx_status(logger):
    """Query Jira for every Jira ID found in ELK-summary.xlsx and update the Status column.

    Only the XLSX is updated; CSV is not touched.
    """
    logger.log("")
    logger.log("=" * 60)
    logger.log("Refresh XLSX Status")
    logger.log("=" * 60)

    xlsx_path = _elk_query.SUMMARY_XLSX
    if not os.path.exists(xlsx_path):
        logger.log(f"  [SKIP] XLSX not found: {xlsx_path}")
        return
    ...
    if not status_col:
        logger.log("  [WARN] XLSX 中找不到 'Status' 欄位，無法更新")
        return

    # Collect all unique valid Jira IDs from the XLSX
    jira_ids = set()
    for row_idx in range(2, ws.max_row + 1):
        for col in filter(None, [jira_id_col, related_col]):
            val = ws.cell(row=row_idx, column=col).value
            if val and re.match(r'^ZNGA-\d+$', str(val).strip()):
                jira_ids.add(str(val).strip())

    logger.log(f"  找到 {len(jira_ids)} 個唯一 Jira ID，開始查詢狀態...")

    # Query Jira for each ID
    status_map = {}
    for i, jid in enumerate(sorted(jira_ids), 1):
        exists, status_text = check_issue_exists(_config, jid)
        status_map[jid] = status_text if exists else ""
        logger.log(f"  ({i}/{len(jira_ids)}) {jid}: {status_text if exists else 'NOT FOUND'}")
    ...
    for row_idx in range(2, ws.max_row + 1):
        ...
        for col in filter(None, [jira_id_col, related_col]):
            val = ws.cell(row=row_idx, column=col).value
            if val and str(val).strip() in status_map:
                jid = str(val).strip()
                break
        if jid and status_map.get(jid):
            ws.cell(row=row_idx, column=status_col, value=status_map[jid])
            updated += 1

    wb.save(xlsx_path)
    logger.log(f"\n  完成：更新 {updated} 筆 Status 欄位")
...
def main():
    ...
    parser.add_argument("--refresh-status", action="store_true", default=False,
                        help="Refresh Status column in ELK-summary.xlsx from Jira (XLSX only)")
    args = parser.parse_args()

    logger = Logger(source=os.path.splitext(os.path.basename(__file__))[0])
    headless = not args.head

    try:
        # ── Refresh status mode ──
        if args.refresh_status:
            refresh_xlsx_status(logger)
            logger.log("\nRefresh Status 完成！")
            logger.close()
            return
